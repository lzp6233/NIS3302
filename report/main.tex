...
\begin{tikzpicture}[
    ...
    % 前端层
    \node[box, fill=blue!20] (web) at (0,4) {Web前端\\HTML5/CSS/JS};
    
    % API层
    \node[box, fill=green!20] (api) at (0,2.5) {RESTful API\\httplib};
    
    % 后端服务层
    \node[box, fill=yellow!20] (backend) at (0,1) {C++后端服务};
    
    % 功能模块层
    \node[box, fill=orange!20] (icmp) at (-3,0) {ICMP模块\\ping.h/cpp};
    \node[box, fill=orange!20] (port) at (0,0) {端口扫描模块\\PortScanner.h/cpp};
    \node[box, fill=orange!20] (network) at (3,0) {网络模块\\network.h/cpp};
    
    % 连接
    \draw[arrow] (web) -- (api);
    \draw[arrow] (api) -- (backend);
    \draw[arrow] (backend) -- (icmp);
    \draw[arrow] (backend) -- (port);
    \draw[arrow] (backend) -- (network);
\end{tikzpicture}
\caption{系统总体架构图}
...
\begin{itemize}
    ...
    \item \textbf{测试结论}：SYN扫描功能正常，扫描速度达到每秒数百个端口
\end{itemize}
